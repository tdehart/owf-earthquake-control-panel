...
var ControlPanel = {
	...
	getQuakes: function() {
		...
		for (var i = 0; i < this.quakes.length; i++) {
			var quake = JSON.parse(this.quakes[i]);
			if (quake.mag >= minMag && quake.mag <= maxMag) {
				filteredQuakes.push(quake);
			}
		}

		filteredQuakes = filteredQuakes.slice(0, $("#slider-earthquake-display").slider("value"));
		console.log(filteredQuakes.length);
		return filteredQuakes.slice(0, $("#slider-earthquake-display").slider("value"));
	}
};
